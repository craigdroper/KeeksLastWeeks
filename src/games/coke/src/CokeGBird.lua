...
function CokeGBird:collides(pipe)
    ...
    if (self.x + 2) + (self.width - 4) >= pipe.x and self.x + 2 <= pipe.x + COKEG_PIPE_WIDTH then
        if (self.y + 2) + (self.height - 4) >= pipe.y and self.y + 2 <= pipe.y + COKEG_PIPE_HEIGHT then
            return true
        end
    end
